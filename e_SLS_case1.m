...
for iter = 1:T
    ...
    vt = min(2*mu/(sqrt(d)*M), min(-fi_current)/(2*L));   % Compute step length for finite difference
    ...
    G0 = (Y_current - y_current)/vt;        % Compute gradient estimator for objective
    GI = (FI_current - fi_current)/vt;      % Compute gradient estimator for constraints
    gt_hist = [gt_hist; G0'];  

    p = -G0;                                % Define the search direction

    % Gradient estimation deviation
    Delta_ub = sqrt(d)*vt*M/2;

...
n_k = 100;      % Number of measurements of function values
...
for iter = 1:T
    ...
    if n_k ==0
        n_k = round(-16*sigma^2*log(delta)/(3*vk^4*M^2));
    end
    ...
    Y_current = Y_current + mean(sigma.*randn([size(Y_current),n_k]),3);
    FI_current = FI_current + mean(sigma.*randn([size(FI_current),n_k]),3);
    ...
    y_k1 = y_k1+mean(sigma.*randn([size(y_k1),n_k]),3);
    fi_k1 = fi_k1+mean(sigma.*randn([size(fi_k1),n_k]),3);
    ...
    y_next = y_next+mean(sigma.*randn([size(y_next),n_k]),3);
    fi_next = fi_next+mean(sigma.*randn([size(fi_next),n_k]),3);
